...
@router.post("")
async def create_session(body: SessionCreate, store: BaseStore = Depends(get_store)):
...
@router.get("")
async def list_sessions(store: BaseStore = Depends(get_store)):
...
async def get_session(session_id: str, store: BaseStore = Depends(get_store)):
    """Get a session by ID."""
...
async def update_session(
    session_id: str, body: SessionUpdate, store: BaseStore = Depends(get_store)
):
    """Update a session."""
    session = await store.get_session(session_id)
    if not session:
...
async def delete_session(session_id: str, store: BaseStore = Depends(get_store)):
